...
export default function DriverPerformance() {
  ...
  // Filter delivered orders within the selected month
  ...
  const driverData = useMemo(() => {
    ...
    // Merge driver names from drivers entity (more reliable)
    map.forEach((d) => {
      const found = drivers.find(dr => dr.id === d.id);
      if (found) d.name = found.name;
    });
  ...
  const isLoading = loadingOrders || loadingDrivers;
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        ...
        <StatCard
          ...
          sub={topDriver ? `${topDriver.orders} orders · ${topDriver.hours}h` : 'No data'}
          ...
          <div className="bg-card border border-border rounded-xl p-5">
            ...
            <ResponsiveContainer width="100%" height={Math.max(220, driverData.length * 48)}>
              <BarChart data={driverData} layout="vertical" margin={{ left: 8, right: 32, top: 4, bottom: 4 }}>
                ...
                <Bar dataKey="orders" name="Orders" radius={[0, 4, 4, 0]} maxBarSize={26}>
                  {driverData.map((_, i) => (
      ...
      {driverData.length > 0 && (
        <div className="bg-card border border-border rounded-xl overflow-hidden">
          <div className="px-5 py-3 border-b border-border">
            <h2 className="font-semibold text-sm">Driver Breakdown — {monthRange.label}</h2>
          </div>
                ...
                <div key={d.id} className="flex flex-wrap items-center gap-x-4 gap-y-1 px-5 py-3 hover:bg-muted/40 transition-colors">
                  ...
                  <div className="flex items-center gap-3 flex-wrap text-sm">
                    <span className="flex items-center gap-1">
                      ...
                      <strong>{d.orders}</strong> <span className="text-muted-foreground text-xs">orders</span>
                    </span>
                    <span className="flex items-center gap-1">
                      <Clock className="w-3.5 h-3.5 text-amber-500" />
                      <strong>{d.hours}h</strong> <span className="text-muted-foreground text-xs">worked</span>
                    </span>
                    <Badge variant="outline" className="text-[10px] px-1.5 py-0 text-muted-foreground">
                      ~{avgHoursPerOrder}h / order
                    </Badge>
                  </div>
                </div>
              );
